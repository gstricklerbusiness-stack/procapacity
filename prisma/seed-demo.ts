...
export async function seedDemoData(workspaceId: string) {
  ...
  console.log(`  Created ${internalProjects.length} internal projects`);
  ...
  console.log(`  Created ${projects.length} client projects`);
  ...
  // Mike: 25h/week on Meridian + 20h/week on TechStart = 45h/40h = 112.5% (red)
  ...
  console.log(`  Created ${assignments.length} billable assignments`);
  ...
  if (internalProject) {
    ...
    console.log("  Created 1 internal assignment for Tom Wilson");
  }
  ...
  console.log("  - Sarah Chen: 87.5% (yellow - near capacity)");
  console.log("  - Mike Johnson: 112.5% (red - OVER capacity)");
  console.log("  - Lisa Park: 62.5% (green - has availability)");
  console.log("  - Tom Wilson: 95% (yellow - near capacity)");
  console.log("  - Emma Davis: 100% (at exact capacity)");
  console.log("  - James Lee: 62.5% (green - has availability)");
...
async function main() {
  ...
  if (!workspaceId) {
    console.error("Usage: tsx prisma/seed-demo.ts <workspace_id>");
    process.exit(1);
  }
